...
export class HpDxDataGridComponent implements OnInit, AfterViewInit {
  ...
  onContextMenuPreparing(e: any): void {
    ...
    if (e.target == "content") {
      ...
      if (this.operationColumnMenuItem != null) {
        let operationMenu = JSON.parse(JSON.stringify(this.operationColumnMenuItem));
        if (e.row == null) {
  ...
  onContextMenuGridClick(e: any): void {
    ...
    let object = {
      e: e, i: { data: this.contextRightClickItemData }
    };
  ...
  @Input()
  columnResizingMode: string = 'widget'//'nextColumn';
  ...
  set operationColumnMenuItem(val: any) {
    ...
    if (this.operationColumnMenuItem != null && this.operationColumnMenuItem[0] != null && this.operationColumnMenuItem[0].items != null) {
      this.contextMenuDataSource = [{ html: "<i class='icon fa-icon-copy fa-menuActionItem'><span class='fa-menuActionTitle'>کپی</span></i>", id: "copyClipboard" }];
      this.contextMenuDataSource = this.contextMenuDataSource.concat(this.operationColumnMenuItem[0].items);
    }
  }
  ...
  onClickItem(e: any, info: any) {
    this.onClickOperationItem.emit({ e: e, i: info });
  }
  ...
  loadChangeDataSource() {
    if (this.changeDataSourceValue != null) {
      this.changeDataSource(this.changeDataSourceValue.model, this.changeDataSourceValue.routingAction, this.changeDataSourceValue.hasAccess);
    }
  }
  ...
  changeDataSource<T>(model: T, routingAction: string, hasAccess: boolean = true): void {
    ...
    let dataSourceModel: any = this.buildDataSourceModel(model, routingAction);
      ...
      store: new CustomStore({
        ...
        load: (loadOptions: LoadOptions<any> | any) => {
          if (this._pagingEnabled && loadOptions.take != null) {
            ...
            model.model.orderByColumn = model.model.orderByColumn == null ? this.orderByColumn : model.model.orderByColumn;
          ...
          var serverData = lastValueFrom(this.httpService.post(model.listUrl, model.model));

          var result = serverData.then((res: any) => {
            this.onAfterChangeDataSource.emit(res);
            let totalCount = res.data.length < 1
              ? 0
            ...
            this.dataGrid!.instance.updateDimensions();
            var data: ResolvedData<any> = { data: res.data, totalCount: totalCount };
            return data;
  ...
  setScroll() {
    if (this.rtlEnabled == true) {
      var width = this.dataGrid!.instance?.getScrollable()?.clientWidth();
  ...
  onOptionChangedExtra(e: any) {
    ...
    if (e.name == 'columns' && !Array.isArray(e.value)) {
      var colnum: number = parseInt(e.fullName.substring(8, e.fullName.length - 7));
  ...
  onShowFixedColumnChanged(e: any): void {
    let columns = this.columns == null ? [] : JSON.parse(JSON.stringify(this.columns));
