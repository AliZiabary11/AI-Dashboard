...
export class HpDxDropDownListComponent implements ControlValueAccessor {
  ...
  @Input('ngModelOptions') ngModelOptions: any = { standalone: true };
  @Input() value?: any;
  ...
  @Input() searchMode: string = 'contains';
  ...
  @Input() stylingMode: string = 'outlined';
      ...
      store: new CustomStore({
        ...
        byKey: (key: any) => {
          model.model[this.valueExpr] = key;
            var res = lastValueFrom(this.httpService
              .post(model.listUrl, model.model));
          return res.then((resData: any) => {
              var res = resData.data[0];
              return res;
            });
        },
      }),
  ...
  selectedItemChangeExtra(e: any): void {
    this.selectedItem = e;
    let value = { value: e && e[this.valueExpr], selectedItem: e };
