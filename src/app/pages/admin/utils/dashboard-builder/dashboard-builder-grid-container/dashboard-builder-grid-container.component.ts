...
export class DashboardBuilderGridContainerComponent implements AfterViewInit {
  ...
  constructor(private viewContainerRef: ViewContainerRef, private readonly cdr: ChangeDetectorRef,private httpService: HttpService){
  ...
  async addDashItem(){
    var res = await this.sendDataToGPT();
  ...
  async sendDataToGPT() {
    ...
    var sendModel = { text: this.reportPrompt, database: this.reportDatabase, gptModel: this.reportGptModel };
