...
import { AlertService, HttpService } from '@hp/core/general';
...
export class GptAssistantComponent implements AfterViewInit {
  ...
  constructor(private httpService: HttpService, private readonly cdr: ChangeDetectorRef, private alertService: AlertService) { }
  ...
  async sendDataToGPT() {
    ...
    var sendModel = { text: this.prompt, database: this.selectedDatabase, gptModel: this.selectedGPTModel };
    var response = await lastValueFrom(this.httpService.rawPost('api/gptdb', sendModel, true));
    ...
    let chtData = this.prepareChartData(this.res_rowData);
    this.barChartData = {
    ...
    this.addChat({question: this.prompt, response: this.currentChatgptResopnse , database: this.selectedDatabase, gptModel: this.selectedGPTModel});
  ...
  prepareChartData(dataList: any) {
    ...
    dataList.forEach((item: any) => {
      ...
      let labelProperties = properties.slice(0, -1); // All properties except the last one for x-axis

      let label = labelProperties.map(prop => item[prop]).join(' '); // Combine properties for label
      labels.push(label);
  ...
  fetchRealtedData() {
    if (this.selectedDatabase == 'Titanic') {
      ...
      this.httpService.rawPost('api/dataq', { tableName: tableName , databaseName: this.selectedDatabase }).subscribe(res => {
        let colDefs = [];
        colDefs = this.generateColDefsFromJson(res);
        this.columnDefs = colDefs;
        this.rowData = res;
...
class ChatHistory{
  constructor(public question: string,public response: any, public gptModel: string, public database: string){}
}
